$(document).ready(function(){
    ...
    <div class= "gnb_btn close"> - 메뉴 닫기 버튼일때
        <button>메뉴닫기</button>
    </div>
    */
    $('.header .gnb .gnb_btn').on('click', function(){
        if($(this).hasClass('open') == true){ //open 클래스를 가지고 있으면 :닫힘 > 열기
            $('.header').addClass('menu_open')
            $(this).removeClass('open')
            $(this).addClass('close')
            $('.header .gnb .gnb_btn button').text('메뉴닫기')

        }else{ // open 클래스가 없을때 : 열림 > 닫힘
    ...
    메뉴 열기 slideDown()
    메뉴 닫기 slideUp
    같은 1차 메뉴를 클릭하는데 클릭할때마다 현재 메뉴가 열린 상태인지  아닌지 판단
    열린상태라면 li에 subb_open이라는 클래스가 있음
    */
