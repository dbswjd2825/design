$(document).ready(function(){
    ...
    let scrolling //헌재 스크롤 된 값
    let window_h //브라우저의 높이
    let offset = 200 //콘텐츠가 하단에서부터 offset 값만큼 올라오면 애니메이션 추가
    ...
    function scroll_chk(){
        ...
        if((sec02_obj_top - scrolling - window_h) < -offset){
            ...
            if((408 - (sec02_obj_top - scrolling - window_h + offset)*1.4) <= $(window).width()){
                $('.sec02 .photo').width(408 - (sec02_obj_top - scrolling - window_h + offset)*1.4)
            }
        }else{
            $('.sec02 .photo').removeClass(class_name)
            $('.sec02 .photo').width(408)
        }
    ...
    $(window).resize(function(){
        scroll_chk() //브라우저 사이즈가 변경될때마다 1번 실행
    })
    $(window).scroll(function(){
        scroll_chk() //브라우저 스크롤 할때마다 1번 실행
    })
